 ...
 * @method     ChildFamily findOneBySendNewsletter(boolean $fam_SendNewsLetter) Return the first ChildFamily filtered by the fam_SendNewsLetter column
 ...
 * @method     ChildFamily requireOneBySendNewsletter(boolean $fam_SendNewsLetter) Return the first ChildFamily filtered by the fam_SendNewsLetter column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 ...
 * @method     ChildFamily[]|ObjectCollection findBySendNewsletter(boolean $fam_SendNewsLetter) Return ChildFamily objects filtered by the fam_SendNewsLetter column
...
abstract class FamilyQuery extends ModelCriteria
{
     ...
     * Example usage:
     * <code>
     * $query->filterBySendNewsletter(true); // WHERE fam_SendNewsLetter = true
     * $query->filterBySendNewsletter('yes'); // WHERE fam_SendNewsLetter = true
     * </code>
     *
     * @param     boolean|string $sendNewsletter The value to use as filter.
    ...
    {
        if (is_string($sendNewsletter)) {
            $sendNewsletter = in_array(strtolower($sendNewsletter), array('false', 'off', '-', 'no', 'n', '0', '')) ? false : true;
        }
